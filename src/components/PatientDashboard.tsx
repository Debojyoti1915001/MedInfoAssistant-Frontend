...
export default function PatientDashboard() {
    ...
    const fetchData = async () => {
      try {
        ...
        if (session && session.patient_id) {
          setUser(session)
          const patientPrescriptions = await getPatientPrescriptions(session.patient_id)
  ...
  const handleFileUpload = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!allowedTypes.includes(file.type)) {
      setUploadError('Only jpg, png, pdf, and txt files are allowed')
      return
    }

    if (file.size > maxSize) {
      setUploadError('File size must be less than 30 MB')
      return
    }
    ...
    try {
      const newPrescription = await submitPrescription(file, symptoms)
          ...
          <form onSubmit={handleFileUpload} className="space-y-4">
            <div>
              ...
              <p className="text-xs text-slate-500 mt-1">
                Allowed: jpg, png, pdf, txt (Max 30 MB)
              </p>
            </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead className="border-b border-slate-200">
                <tr>
                  <th className="text-left py-3 px-4 font-semibold text-navy-900">File Name</th>
                  ...
                  <th className="text-left py-3 px-4 font-semibold text-navy-900">Size</th>
                  <th className="text-left py-3 px-4 font-semibold text-navy-900">Action</th>
                </tr>
              </thead>
              <tbody>
                {prescriptions.map((prescription) => (
                  <tr key={prescription._presId} className="border-b border-slate-100 hover:bg-slate-50">
                    <td className="py-3 px-4 text-slate-700">{prescription.file.name}</td>
                    ...
                    <td className="py-3 px-4">
                      <button
                        onClick={() => downloadPrescriptionFile(prescription.file.path, prescription.file.name)}
